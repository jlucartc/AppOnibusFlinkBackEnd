...
class Manager extends Thread {
    ...
    override def run(): Unit = {
        ...
        val bootstrapServers = sys.env.get("GITHUB_JLUCARTC_KAFKATOPICMESSAGEPRODUCER_BOOTSTRAP_SERVERS") match { case Some(res) => { res } case None => { "" } }
        val keySerializer = sys.env.get("GITHUB_JLUCARTC_KAFKATOPICMESSAGEPRODUCER_KEY_SERIALIZER") match { case Some(res) => { res } case None => { "" } }
        val valueSerializer  = sys.env.get("GITHUB_JLUCARTC_KAFKATOPICMESSAGEPRODUCER_VALUE_SERIALIZER") match { case Some(res) => { res } case None => { "" } }
        val acks = sys.env.get("GITHUB_JLUCARTC_KAFKATOPICMESSAGEPRODUCER_ACKS") match { case Some(res) => { res } case None => { "" } }
